...
var customAttributes = [
  ...
  'strip' // strip the tag from the output
];
...
Parser.prototype.onOpenTag = function(tagName, attribs) {
  ...
  if (_.has(attribs, this._customAttrs.strip)) {
  ...
  if (_.isEmpty(this._options.assetPathPrefix) === false) {
    if (tagName === 'link' && attribs.href) {
      ...
      // Don't prepend the asset path if the tag has the special data-src-keep
      // attribute or it is an embedded image where the src starts with "data:"
      var shouldPrependPath = !_.has(attribs, this._customAttrs['src-keep']) &&
        attribs.src.slice(0, 5) !== 'data:';
...
Parser.prototype.prependAssetPath = function(attribs, pathAttr) {
  var assetPath = attribs[pathAttr];
  if (_.isEmpty(assetPath)) return;

  // If the path is already absolute, leave it as-is.
  if (absoluteUrlRe.test(assetPath)) return;

  var adjustedPath;
  if (assetPath[0] === '.' && !_.isEmpty(this._options.pathFromRoot)) {
    adjustedPath = this.slashJoin(this._options.assetPathPrefix, this._options.pathFromRoot, assetPath);
  } else {
    adjustedPath = this.slashJoin(this._options.assetPathPrefix, assetPath);
  }

  attribs[pathAttr] = adjustedPath;
};

Parser.prototype.slashJoin = function() {
