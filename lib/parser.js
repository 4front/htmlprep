  ...
  this._parser = new HtmlParser({
    ...
    ontext: function(text) {
      ...
      if (text.length === 1 && charEntityReferences[text]) {
        ...
        self._output.push(self._replaceBaseUrlPlaceholder(text));
      }
    },
...
Parser.prototype._replaceBaseUrlPlaceholder = function(text) {
  var self = this;
  // If the match ends with a slash, make sure the baseUrlPlaceholder
  // also ends with a slash. The baseUrlPlaceholder is guaranteed
  // not to end with a slash.
  return text.replace(this._baseUrlRegex, function(match) {
    if (endsWith(match, '/')) {
      return self._options.baseUrl + '/';
    }
    return self._options.baseUrl;
  });
};
